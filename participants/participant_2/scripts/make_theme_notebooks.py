...
def nb7():
    cells = [
        ...
        md(
            ...
            "Помимо технической MAE-модели на всех данных обучена и **осторожная продуктовая** "
            "версия с `loss=Quantile:alpha=0.40` (raw-таргет) — скрипт "
            "`scripts/final_full_quantile.py`. Quantile сознательно занижает прогноз, жертвуя "
            "общим MAE ради продуктовых/short-risk метрик. Сравниваем обе full-версии."
        ),
        code(
            "rows = []\n"
        ...
        md(
            "**Вывод.** Quantile 0.40 ожидаемо **хуже по общему MAE**, но **лучше по ProductMAE / "
            "EngagementRiskMAE / small_mae** — осторожный прогноз снижает переоценку коротких "
            "сессий. Это продуктовая модель для сценариев, где переоценить вовлечённость "
            "опаснее (реклама/CRM). Выбор: техническая точность → MAE-модель; short-risk → "
            "Quantile 0.40. Артефакты — `outputs/final_models_full/*_q040.cbm`."
        ),
    ]
